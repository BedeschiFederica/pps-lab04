...
object Ex1ComplexNumbers:
  ...
  object BasicComplexADT extends ComplexADT:
    ...
    extension (complex: Complex)
      def re(): Double = complex match {case ComplexImpl(re, im) => re}
      def im(): Double = complex match {case ComplexImpl(re, im) => im}
      ...
      def asString(): String = complex match
        case ComplexImpl(re, im) if re != 0 && im > 0 => s"$re + ${im}i"
        case ComplexImpl(re, im) if re != 0 && im < 0 => s"$re - ${-im}i"
        case ComplexImpl(re, im) if im != 0 => s"${im}i"
        case ComplexImpl(re, im) => s"$re"
